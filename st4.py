    ...
    try:
        if not EC.title_is(u"房地产估价报告网上备案管理系统")(brow):
            if i1>1:
                ...
                print(11)
                brow.get("http://183.194.243.146:7001/fangdi/system/USBLogin.jsp")
                handle = brow.current_window_handle
            
            input_loc = ("name","keypwd")
            WebDriverWait(brow,10).until(lambda x:x.find_element(*input_loc)).send_keys("bfc4204e")
...
def connect(datau):
    global t1 
    t1= False
    path=r"E:\study\Scripts\IEDriverServer.exe"
    ...
    #print(browser.page_source)
        ...
        #是否分户 是分户选择地址
            ...
            WebDriverWait(browser,10).until(lambda x:x.find_element_by_xpath("//input[@value='提交']")).click()
...
#houseReside 20旧式里弄 3公寓 6公寓1  15简屋  7公寓2  8花园住宅 16新工房 17集宿 18住宅 9新式里弄 19职工宿舍 10新工房1  11新工房2 12新工房3  13旧式里弄1 14旧式里弄2
