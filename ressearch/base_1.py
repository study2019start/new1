...
class base(object):
    ...
    def wirte_excel(self,nno,listno,filename,lsp=3):
        self.logindd(nno)
        resultln=self.tudisearch(listno,lsp)
        df=pd.DataFrame(resultln)
        model_excel.xlwingcreate(df,filename,"B2")
    ...
    def logindd(self,no):
        ...
        driver = webdriver.Chrome(options = chrome_options,executable_path=r"E:\study\chromedriver.exe")
    ...
    def tudisearch(self,listno,ls=3): #列表[开始时间，结束时间，交易状态，类型]
                                    ...
                                    if tdlist:
                                        ...
                                        if buttonn:
                                            ...
                                            if el_dialog:
                                                ...
                                                dicr['qu']=self.get_eleminxpath(zilist[0],'.//span').text
                                                dicr['bankuai']=self.get_eleminxpath(zilist[1],'.//span').text
                                                dicr['huanxian']=self.get_eleminxpath(zilist[2],'.//span').text
                                                ...
                                                dicr['zongjia']=self.get_eleminxpath(zilist[13],'.//span').text
...
if __name__ == "__main__":
    ...
    filename=r"E:\tudilou.xls"
    ls=['2014-01-01','2020-12-11','成交','工业']#列表[开始时间，结束时间，交易状态，类型] #土地成交导入access
    #search.insert_access(no1,ls,'bj_table',databaur)
    search.wirte_excel(no1,ls,filename,150)
    #################################################################
